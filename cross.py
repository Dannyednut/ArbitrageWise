...
class CrossExchange(Engine):
    ...
    async def start_price_streams(self):
        ...
        tasks.append(
            asyncio.create_task(self.continuous_opportunity_detection()))
    ...
    async def stream_manager(self, exchange_name: str, exchange, symbol: str):
        ...
        while self.engine.running:
            try:
                ...
                logger.error(
                    f"[{exchange_name}] Unhandled exception in stream for {symbol}: {e}. Reconnecting in {self.reconnect_delay}s."
                )
            ...
            try:
                for symbol in self.symbols:
                    ...
                    for opp in opportunities:
                        ...
                        logger.info(
                            f"Cross-Exchange Opportunity: {opp.trading_pair} | Profit: {opp.profit_percentage:.3f}%"
                        )
                        await self.engine.save_opportunity(
                ...
                logger.error(
                    f"Error in continuous opportunity detection loop: {e}")
    ...
    def analyze_symbol_opportunities(self, symbol: str) -> list:
        ...
        for exchange_name, exchange_data in self.price_cache.items():
            if symbol in exchange_data and exchange_data[symbol].get(
                    'bid') and exchange_data[symbol].get('ask'):
                prices.append({
    ...
    def _find_arbitrage_opportunities(
            self, price_data: List[Dict]) -> List[Opportunity]:
        ...
        valid_prices = [
            p for p in price_data if p and p.get('bid') and p.get('ask')
        ]
        if len(valid_prices) < 2:
            return opportunities

        sorted_by_ask = sorted(valid_prices, key=lambda x: x['ask'])
        sorted_by_bid = sorted(valid_prices,
                               key=lambda x: x['bid'],
                               reverse=True)
    ...
    async def _execute_cross_exchange_trade(self, op: dict,
                                            req: dict) -> TradeResult:
    ...
    async def _execute_instant_arbitrage(self, op: dict,
                                         amount_usd: float) -> TradeResult:
        buy_ex_name, sell_ex_name = op['buy_exchange'], op['sell_exchange']
        buy_exchange, sell_exchange = self.engine.exchanges.get(
            buy_ex_name), self.engine.exchanges.get(sell_ex_name)
        pair = op['trading_pair']
        base, quote = pair.split('/')

        if not all([buy_exchange, sell_exchange]):
            return TradeResult("error",
                               "One of the exchanges is not initialized.")

        # 1. Pre-Trade Checks
        if self.engine.account_balances.get(buy_ex_name,
                                            {}).get(quote, 0) < amount_usd:
            return TradeResult("error",
                               f"Insufficient {quote} on {buy_ex_name}")

        amount_to_trade_base = amount_usd / op['buy_price']
        if self.engine.account_balances.get(sell_ex_name,
                                            {}).get(base,
                                                    0) < amount_to_trade_base:
            return TradeResult("error",
                               f"Insufficient {base} on {sell_ex_name}")

        # 2. Execute Orders
        try:
            logger.info(
                f"⚡️ ATTEMPTING INSTANT ARBITRAGE: {pair} | {amount_usd:.2f} {quote}"
            )
            buy_task = buy_exchange.create_market_buy_order(
                pair, amount_to_trade_base)
            ...
            if isinstance(buy_order, Exception) or isinstance(
                    sell_order, Exception):
                # Critical: Handle partial failure
                logger.error(
                    f"PARTIAL FAILURE: Buy: {buy_order} | Sell: {sell_order}")
                # Here you would add logic to manually fix the position (e.g., sell what you just bought)
                return TradeResult(
                    "error",
                    f"Partial failure. Manual intervention required. Buy: {buy_order}, Sell: {sell_order}"
                )

            # 3. Log Success
            profit = (sell_order['cost'] - buy_order['cost']) - (
                sell_order.get('fee', {}).get('cost', 0) +
                buy_order.get('fee', {}).get('cost', 0))
            log = {
            ...
            return TradeResult("success",
                               "Instant arbitrage executed successfully.",
                               trade_id, profit)

        except Exception as e:
            logger.error(
                f"Error during instant execution: {e}\n{traceback.format_exc()}"
            )
            return TradeResult("error", str(e))

    async def _execute_transfer_arbitrage(self, op: dict,
                                          amount_usd: float) -> TradeResult:
        try:
            buy_ex_name, sell_ex_name = op['buy_exchange'], op['sell_exchange']
            ...
            quote_balance = self.account_balances.get(buy_ex_name,
                                                      {}).get(quote, 0)
            if quote_balance < amount_usd:
                return TradeResult(
                    "error",
                    f"Insufficient {quote} on {buy_ex_name} (have: {quote_balance})"
                )

            # --- 1a. Estimate Slippage ---
            slip = await self.engine.calc_slippage(buy_exchange, pair, 'BUY',
                                                   Decimal(str(amount_base)))
            if not self.engine._slippage_ok(slip):
                return TradeResult(
                    "error",
                    f"Buy slippage too high on {buy_ex_name}: {slip:.4%}")

            # --- 2. Place Market Buy Order ---
            amount_base = amount_usd / op['buy_price']
            logger.info(
                f"[TRANSFER ARB] Buying {amount_base:.6f} {base} on {buy_ex_name} at {op['buy_price']}"
            )
            buy_order = await buy_exchange.create_market_buy_order_with_cost(
                pair, quote_balance)

            if not buy_order or 'status' not in buy_order or buy_order[
                    'status'] != 'closed':
            ...
            logger.info(
                f"[TRANSFER ARB] Fetching deposit address for {base} on {sell_ex_name}"
            )
            common_chains = await self.get_common_transfer_chains(
                base, buy_exchange, sell_exchange)

            if not common_chains:
                return TradeResult(
                    "error",
                    f"No common transfer chains for {base} between {buy_ex_name} and {sell_ex_name}"
                )

            best_chain, fee = await self.select_best_transfer_chain(
                common_chains, buy_exchange, base)
            if not best_chain:
                return TradeResult(
                    "error",
                    f"Could not determine best transfer chain for {base}")

            deposit_info = await sell_exchange.fetch_deposit_address(
                base, {'network': best_chain})
            address = deposit_info['address']
            tag_or_memo = deposit_info.get(
                'tag', None)  # Some exchanges require this (e.g., XRP, XLM)

            # --- 4. Withdraw from Buy Exchange ---
            logger.info(
                f"[TRANSFER ARB] Withdrawing {amount_base:.6f} {base} from {buy_ex_name} to {sell_ex_name}"
            )
            ...
            logger.info(
                f"[TRANSFER ARB] Waiting for deposit of {amount_base:.6f} {base} on {sell_ex_name}"
            )
            ...
            while waited < max_wait:
                ...
                if balance['total'].get(
                        base,
                        0) >= (amount_base - fee) * 0.99:  # Allow small margin
                    logger.info(
                        f"[TRANSFER ARB] Deposit received on {sell_ex_name}")
                ...
                return TradeResult(
                    "error", "Timeout waiting for deposit on sell exchange.")

            # --- 6a. Estimate Slippage ---
            slip = await self.engine.calc_slippage(
                sell_exchange, pair, 'SELL', Decimal(str(amount_received)))
            if not self.engine._slippage_ok(slip):
                return TradeResult(
                    "error",
                    f"Sell slippage too high on {sell_ex_name}: {slip:.4%}")

            # --- 6b. Sell on Sell Exchange ---
            logger.info(
                f"[TRANSFER ARB] Selling {amount_base:.6f} {base} on {sell_ex_name}"
            )
            sell_order = await sell_exchange.create_market_sell_order(
                pair, amount_received)
            ...
            logger.info(
                f"[TRANSFER ARB] Transfer arbitrage successful. Profit: ${profit:.2f}"
            )
            return TradeResult("success",
                               "Transfer arbitrage executed successfully.",
                               trade_id, profit)
    ...
    async def get_common_transfer_chains(self, asset: str, buy_exchange,
                                         sell_exchange) -> list[str]:
    ...
    async def select_best_transfer_chain(self, common_chains: list,
                                         buy_exchange, asset: str) -> str:
        """
    ...
    async def _log_pending_transfer(self, op: dict,
                                    amount_usd: float) -> TradeResult:
        ...
        log = {
            ...
            "quantity": amount_usd / op['buy_price'],
            "profit_usd": op['profit_usd'],
            "status": "pending",
            "strategy": "transfer"
        }
        trade_id = await self.engine._log_trade_to_base44(log)
        if trade_id:
            return TradeResult(
                "success",
                "Trade logged as 'pending'. Manual transfer required.",
                trade_id)
    ...
    async def start(self):
        ...
        if not self.engine.exchanges:
            logger.error(
                "No exchanges configured. Stopping cross-exchange engine.")
            return
