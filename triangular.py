...
class Triangular:
    ...
    async def exchange_scanner_loop(self, exchange_name: str,
        ...
        if not all_paths:
            logger.warning(
                f"[{exchange_name}] No triangular paths found. Stopping scanner."
            )
            return

        unique_symbols = self._build_path_lookup_map(exchange_name, all_paths)
        logger.info(
            f"[{exchange_name}] Generated {len(all_paths)} paths across {len(unique_symbols)} symbols."
        )
        ...
        try:
            ...
            for symbol in symbols_to_watch:
                task = asyncio.create_task(
                    self._watch_symbol(exchange_name, exchange, symbol))
                tasks.append(task)
            ...
            if tasks:
                ...
                logger.warning(
                    "No symbol available to watch for triangular scanning")
    ...
    async def _calculate_path_profit(self, exchange_name: str, exchange,
        ...
        try:
            ...
            if not (ob1.get('asks') and ob2.get('asks') and ob3.get('bids')
                    and len(ob1['asks']) > 0 and len(ob2['asks']) > 0
                    and len(ob3['bids']) > 0):
                return
    ...
    def _build_path_lookup_map(self, exchange_name: str,
                               all_paths: list) -> list:
    ...
    async def _generate_triangular_paths(self, exchange) -> list:
        ...
        try:
            ...
            spot_markets = {
                symbol: market
                for symbol, market in markets.items() if market.get('spot')
                and market.get('active') and '/' in symbol
            }

            logger.info(
                f"[{exchange.id}] Found {len(spot_markets)} active spot markets."
            )
            ...
            for pair1 in spot_markets:
                ...
                for pair2 in asset_pairs.get(asset2, []):
                    ...
                    pair3_options = [
                        f"{asset3}/{asset1}", f"{asset1}/{asset3}"
                    ]
                    for pair3 in pair3_options:
                        if pair3 in spot_markets:
                            valid_paths.add(
                                (asset1, asset2, asset3, pair1, pair2, pair3))
                            break

            logger.info(
                f"[{exchange.id}] Generated {len(valid_paths)} total triangular paths."
            )
            ...
            logger.info(
                f"[{exchange.id}] Filtered to {len(filtered_paths)} priority paths."
            )
            ...
            logger.info(
                f"[{exchange.id}] Failed to generate triangular paths: {e}")
            return []
    ...
    async def _execute_triangular_trade(self, op: dict,
                                        req: dict) -> TradeResult:
        exchange_name = op['exchange']
        exchange: Optional[ccxt.Exchange] = self.engine.exchanges.get(
            exchange_name)
        ...
        try:
            ...
            amount2 = await self.try_trade_leg(exchange, pair1, asset1,
                                               amount1)
            ...
            amount3 = await self.try_trade_leg(exchange, pair2, asset2,
                                               amount2)
            ...
            final_amount = await self.try_trade_leg(exchange, pair3, asset3,
                                                    amount3)
            if isinstance(final_amount,
                          TradeResult) and final_amount.status == "error":
                return final_amount
            ...
            return TradeResult("success",
                               "Triangular trade executed successfully.",
                               trade_id, profit)

        except Exception as e:
            logger.error(
                f"Error during triangular execution: {e}\n{traceback.format_exc()}"
            )
            return TradeResult(
                "error",
                f"Failed on one of the legs: {e}. Manual intervention required."
            )
    ...
    async def start(self):
        ...
        logger.info(
            f"Initialized {len(self.engine.exchanges)} exchanges: {list(self.engine.exchanges.keys())}"
        )
